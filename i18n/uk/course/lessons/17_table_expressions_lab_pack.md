...
5. Чому “лікувати дублікати через `DISTINCT`” часто є code smell у проді?
6. Яка форма індексу допомагає масштабувати патерн APPLY з Лабу 4 (seek + упорядкований TOP)?
7. У Лабі 6: що переконає вас, що оптимізатор переписав correlated subquery у set‑based план?

## Домашнє (tradeoffs: коректність vs продуктивність vs підтримуваність)

1. Розв’яжіть **Лаб 4** через **window functions** (як у Лабі 3) і включіть клієнтів без замовлень.
  - Який варіант легше довести як коректний?
  - Який варіант простіше розширити (наприклад: додати ще “останній час PAID”)?

2. Підсиліть кейс з ties: додайте ще одне замовлення для Customer 1 з тим самим `OrderDateTime`, але іншим `OrderID`.
  - Покажіть, як ламається недетермінований top‑1.
  - Виправте через детермінований tie‑breaker і сформулюйте правило.

3. Виміряйте plan shape: для **Лабу 6** запустіть запит з і без рекомендованого індексу.
  - Збережіть `STATISTICS IO, TIME` і Actual Execution Plan.
  - Коротко поясніть, що змінилось (оператори, join type, seek/scan) і чому.

4. Крос‑DB мислення (опційно): перепишіть Лаб 4 і Лаб 5 для PostgreSQL через `LATERAL`.
  - Зафіксуйте відмінності синтаксису й що б ви робили з індексацією.
